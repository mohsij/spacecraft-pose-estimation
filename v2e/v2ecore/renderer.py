...
class EventRenderer(object):
    ...
    def render_events_to_frames(self, event_arr: np.ndarray,
                                ...
                                return_frames=False,
                                output_to_images=False) -> np.ndarray:
        ...
        while not doneWithTheseEvents:
            ...
            if not doneWithTheseEvents:
                ...
                if self.video_output_file:
                    ...
                    if output_to_images:
                        cv2.imwrite(os.path.join(self.output_path, 'event-frames', '{:.0f}.bmp'.format(t)), cv2.cvtColor((img * 255).astype(np.uint8), cv2.COLOR_GRAY2BGR))
                if self.preview:
